...
import { escapeHtml, localDateKey, seededShuffle } from './utils.js';
import { renderExerciseJapanese, renderJapanese, renderWordJapanese } from './japanese.js';
...
function supportLabel(day) {
	if (day <= 20) return '漢字完整標示讀音';
	if (day <= 40) return '新漢字優先標示';
	if (day <= 60) return '近期漢字保留讀音';
	if (day <= 80) return '逐步降低讀音提示';
	if (day <= 90) return '少量讀音提示';
	return '模擬區不標讀音';
}

function supportBadge(day) {
	return `<span class="support-badge"><i class="bi bi-spellcheck"></i> ${supportLabel(day)}</span>`;
}
...
export function renderWordCard(word, day, state) {
	const level = state.vocab[wordKey(word)] || 0;
	return `<article class="word-card">
		<div class="d-flex justify-content-between gap-3">
			<div class="flex-grow-1">
				<div class="d-flex align-items-center flex-wrap gap-2">
					<span class="word-main">${renderWordJapanese(word, day, state)}</span>
				...
				<div class="btn-group btn-group-sm familiar mt-2" data-word-key="${escapeHtml(wordKey(word))}">
...
export function renderHome(state) {
	...
	return `<section class="hero mb-4">
		<div class="hero-art hero-art-one"></div>
		<div class="hero-art hero-art-two"></div>
		<div class="row align-items-center g-4 position-relative z-1">
			<div class="col-lg-8">
				...
				<div class="eyebrow mb-2">今日學習</div>
			...
			<div class="col-lg-4 d-none d-lg-block">
				<div class="hero-day-card"><div class="hero-day-card-label">目前進度</div><div class="hero-day-card-number">${String(state.currentDay).padStart(2, '0')}</div><div class="hero-day-card-jp">日本語</div></div>
			</div>
		</div>
	</section>
	<div class="row g-3 mb-4">
		${stats.map((item) => `<div class="col-6 col-lg-3"><article class="app-card stat-card p-3 p-md-4 h-100"><div class="stat-icon"><i class="bi ${item[0]}"></i></div><div class="stat-number mt-3">${item[2]}<small>${item[3]}</small></div><div class="stat-label">${item[1]}</div></article></div>`).join('')}
	</div>
	<div class="row g-3 g-lg-4">
		<div class="col-lg-8">
			<section class="app-card today-card p-3 p-md-4 h-100">
				<div class="d-flex justify-content-between align-items-start gap-3 mb-3">
					<div><div class="section-kicker">今日進度</div><h2 class="h3 section-title mt-1 mb-1">${escapeHtml(lesson.title)}</h2><p class="text-body-secondary mb-0">${escapeHtml(lesson.canDo)}</p></div>
					<div class="d-none d-sm-block">${supportBadge(lesson.day)}</div>
				</div>
				<div class="progress-steps">
					${[
			['bi-book', '文法與句型', parts.grammar],
			['bi-translate', '20 張單字', parts.vocab],
			['bi-pencil-square', practiceLabel(stage), parts.practice],
			['bi-patch-check', `${stage.quizCount} 題測驗`, parts.quiz]
		].map((item, index) => `<div class="progress-step ${item[2] ? 'done' : ''}"><span class="step-icon"><i class="bi ${item[0]}"></i></span><div class="flex-grow-1"><small>STEP ${index + 1}</small><b>${item[1]}</b></div><span class="step-state">${item[2] ? '<i class="bi bi-check-lg"></i>' : index + 1}</span></div>`).join('')}
				</div>
		...
		<div class="col-lg-4">
			<aside class="app-card stage-panel p-3 p-md-4 h-100">
				<div class="section-kicker">學習階段</div>
				<h2 class="h4 fw-bold mt-2">${escapeHtml(stage.name)}</h2>
				<div class="stage-days">DAY ${stage.from} — ${stage.to}</div>
				<p class="text-body-secondary mt-3">${escapeHtml(stage.description)}</p>
				<div class="stage-mini-rule"><i class="bi bi-lightbulb"></i><span>教材提供讀音輔助；每日練習與測驗不顯示振假名。</span></div>
				<button class="btn btn-soft w-100 mt-3" id="open-settings-button"><i class="bi bi-sliders me-1"></i>學習設定</button>
			</aside>
		</div>
	</div>`;
}
...
function readingBlock(lesson, day, exam = false) {
	const reading = exam && lesson.exam?.readings?.length ? lesson.exam.readings[0] : buildReadingPractice(lesson, day);
...
function renderPractice(lesson, stage, parts, day) {
	...
	if (stage.id === 'foundation') {
		...
		return `<div class="small text-body-secondary">03 基礎練習</div><h2 class="h4 section-title">先把句型用對</h2><div class="stage-note mb-3"><b>Day 1–10 不做正式讀解／聽解。</b><br><span>先看懂句型、能念例句、認得今天的單字，避免一開始負荷過高。</span></div><div class="mini-practice"><div class="mini-label">看日文選中文</div><div class="jp-text exercise-jp mb-2">${renderExerciseJapanese(example.jp)}</div>${choiceButtons([example.zh, ...previousTranslations(day, 3)], 0, 'practice-choice', day)}</div><button class="btn ${done ? 'btn-success' : 'btn-outline-primary'} w-100 mt-3 fw-bold" id="complete-practice-button">${done ? '✓ 基礎練習完成' : '確認答案並完成'}</button>`;
	}
	if (stage.id === 'sentence') {
		const first = lesson.examples[0];
		const second = lesson.examples[Math.min(1, lesson.examples.length - 1)];
		return `<div class="small text-body-secondary">03 句子建立</div><h2 class="h4 section-title">句型＋單句聽辨</h2><div class="stage-note mb-3">現在才加入<strong>單句</strong>聽辨，仍然不要求完整對話或長篇閱讀。</div><div class="mini-practice mb-3"><div class="mini-label">句子理解</div><div class="jp-text exercise-jp mb-2">${renderExerciseJapanese(second.jp)}</div>${choiceButtons([second.zh, ...previousTranslations(day, 3)], 0, 'practice-choice', day)}</div><div class="listening-mini"><div class="mini-label">聽一句，選意思</div><button class="btn listen-btn mb-2" data-speak="${escapeHtml(first.jp)}"><i class="bi bi-volume-up-fill"></i> 播放一句</button>${choiceButtons([first.zh, ...previousTranslations(day, 3)], 0, 'listen-choice', day)}</div><button class="btn ${done ? 'btn-success' : 'btn-outline-primary'} w-100 mt-3 fw-bold" id="complete-practice-button">${done ? '✓ 今日練習完成' : '確認答案並完成'}</button>`;
	}
	if (stage.id === 'micro') {
		const first = lesson.examples[0];
		const second = lesson.examples[Math.min(1, lesson.examples.length - 1)];
		const text = `${first.jp} ${second.jp}`;
		return `<div class="small text-body-secondary">03 短句理解</div><h2 class="h4 section-title">超短閱讀＋短句聽辨</h2><p class="small text-body-secondary">閱讀先控制在 1–2 句，題目只使用已經學過的句型。</p><div class="reading-box p-3 mb-3"><div class="jp-text exercise-jp">${renderExerciseJapanese(text)}</div><div class="mt-3 fw-semibold">第二句的意思是？</div>${choiceButtons([second.zh, ...previousTranslations(day, 3)], 0, 'practice-choice', day)}</div><div class="listening-mini"><div class="mini-label">聽第一句</div><button class="btn listen-btn mb-2" data-speak="${escapeHtml(first.jp)}"><i class="bi bi-volume-up-fill"></i> 播放</button>${choiceButtons([first.zh, ...previousTranslations(day, 3)], 0, 'listen-choice', day)}</div><button class="btn ${done ? 'btn-success' : 'btn-outline-primary'} w-100 mt-3 fw-bold" id="complete-practice-button">${done ? '✓ 短句理解完成' : '確認答案並完成'}</button>`;
	}
	if (stage.id === 'integrated') {
		return `<div class="small text-body-secondary">03 初級整合</div><h2 class="h4 section-title">短文＋短對話</h2><p class="small text-body-secondary">從 Day 31 開始才正式練短文與短對話；答完再看中文解析。</p>${readingBlock(lesson, day)}${listeningBlock(lesson, day)}<button class="btn ${done ? 'btn-success' : 'btn-outline-primary'} w-100 mt-3 fw-bold" id="complete-practice-button">${done ? '✓ 整合練習完成' : '完成閱讀與聽辨'}</button>`;
	}
	if (stage.id === 'jlpt_intro') {
		return `<div class="small text-body-secondary">03 N5 題型導入</div><h2 class="h4 section-title">讀解・聽解入門</h2><p class="small text-body-secondary">開始靠近 JLPT 題型。練習不顯示振假名，答題後再看中文解析。</p>${readingBlock(lesson, day)}${listeningBlock(lesson, day)}<button class="btn ${done ? 'btn-success' : 'btn-outline-primary'} w-100 mt-3 fw-bold" id="complete-practice-button">${done ? '✓ N5 入門練習完成' : '完成 N5 入門練習'}</button>`;
	}
	if (stage.id === 'exam') {
		return `<div class="small text-body-secondary">03 N5 題型訓練</div><h2 class="h4 section-title">讀解・聽解混合</h2><p class="small text-body-secondary">普通形與核心句型建立後，開始處理更完整的 N5 題型。</p>${readingBlock(lesson, day)}${listeningBlock(lesson, day)}<button class="btn ${done ? 'btn-success' : 'btn-outline-primary'} w-100 mt-3 fw-bold" id="complete-practice-button">${done ? '✓ 題型練習完成' : '完成今日題型練習'}</button>`;
	}
	return `<div class="small text-body-secondary">03 ${stage.id === 'final' ? '綜合模擬' : '模擬題組'}</div><h2 class="h4 section-title">降低提示，接近考試</h2><p class="small text-body-secondary">題目固定不顯示振假名，閱讀與聽解改用獨立的模擬素材。</p>${readingBlock(lesson, day, true)}${listeningBlock(lesson, day, true)}<button class="btn ${done ? 'btn-success' : 'btn-outline-primary'} w-100 mt-3 fw-bold" id="complete-practice-button">${done ? '✓ 模擬練習完成' : '完成今日模擬練習'}</button>`;
}
...
export function renderLesson(day, state) {
	...
	return `<div class="lesson-toolbar mb-3">
		<button class="lesson-nav-btn" id="previous-day-button" aria-label="上一天"><i class="bi bi-chevron-left"></i></button>
		<div class="text-center">${stageBadge(stage)}<div class="lesson-day-label mt-1">DAY ${day} / 100</div></div>
		<button class="lesson-nav-btn" id="next-day-button" aria-label="下一天"><i class="bi bi-chevron-right"></i></button>
	</div>
	<header class="lesson-head app-card lesson-banner p-3 p-md-4 mb-3 mb-lg-4">
		<div class="section-kicker">MODULE ${lesson.module} · ${escapeHtml(lesson.phase)}</div>
		<h1 class="lesson-mobile-title fw-bold mt-2 mb-2">${escapeHtml(lesson.title)}</h1>
		<div class="lesson-pattern-preview">${escapeHtml(lesson.pattern)}</div>
		<div class="d-flex gap-2 flex-wrap mt-3">${supportBadge(day)}<span class="support-badge"><i class="bi bi-pencil-square"></i> 練習不顯示振假名</span><span class="support-badge"><i class="bi bi-ui-checks"></i> ${stage.quizCount} 題測驗</span></div>
		<div class="learning-goal mt-3"><span class="goal-icon"><i class="bi bi-bullseye"></i></span><div><small>今天學完要會</small><b>${escapeHtml(lesson.canDo)}</b></div></div>
	</header>
	<div class="row g-3 g-lg-4">
		<div class="col-lg-7">
			<section class="app-card lesson-section grammar-section p-3 p-md-4 mb-3 mb-lg-4">
				<div class="d-flex justify-content-between"><h2 class="h4 section-title"><span class="section-number">01</span> 文法與句型</h2>${parts.grammar ? '<span class="text-success fw-bold">✓</span>' : ''}</div>
				<div class="pattern-box p-3 my-3"><div class="small text-body-secondary mb-1">今日句型</div><div class="pattern-text">${renderJapanese(lesson.pattern, day, state)}</div></div>
				<p class="mb-3">${escapeHtml(lesson.explanation)}</p>
				${lesson.notes.length ? `<div class="grammar-note p-3 mb-3"><div class="fw-bold mb-2"><i class="bi bi-lightbulb me-1"></i>中文重點</div><ul class="mb-0 ps-3">${lesson.notes.map((note) => `<li class="mb-2">${escapeHtml(note)}</li>`).join('')}</ul></div>` : ''}
				<div class="fw-bold mb-1">例句</div>
				${lesson.examples.map((example) => `<div class="example-row d-flex justify-content-between gap-3"><div><div class="jp-text">${renderJapanese(example.jp, day, state)}</div><div class="example-zh">${escapeHtml(example.zh)}</div></div><button class="btn btn-light speak-btn" data-speak="${escapeHtml(example.jp)}"><i class="bi bi-volume-up"></i></button></div>`).join('')}
				<button class="btn ${parts.grammar ? 'btn-success' : 'btn-primary'} w-100 mt-2 fw-bold" id="complete-grammar-button">${parts.grammar ? '✓ 文法已完成' : '完成文法學習'}</button>
			</section>
			<section class="app-card lesson-section vocab-section p-3 p-md-4 mb-3 mb-lg-4">
				<div class="d-flex justify-content-between align-items-center"><h2 class="h4 section-title mb-0"><span class="section-number">02</span> 今日單字</h2><span class="small text-body-secondary">新 ${newCount} · 複習 ${lesson.vocabulary.length - newCount}</span></div>
				<p class="small text-body-secondary mt-2 mb-0">每天約 20 張；前段混合新字與間隔複習，後段逐步提高複習比重。</p>
				<div class="mt-1">${lesson.vocabulary.map((word) => renderWordCard(word, day, state)).join('')}</div>
				<button class="btn ${parts.vocab ? 'btn-success' : 'btn-primary'} w-100 mt-3 fw-bold" id="complete-vocabulary-button">${parts.vocab ? '✓ 單字已完成' : `完成今日 ${lesson.vocabulary.length} 張`}</button>
			</section>
		</div>
		<div class="col-lg-5">
			<section class="app-card lesson-section practice-section p-3 p-md-4 mb-3 mb-lg-4">${renderPractice(lesson, stage, parts, day)}</section>
			<section class="app-card p-3 p-md-4 mb-3 mb-lg-4">
				<div class="d-flex justify-content-between align-items-start gap-3"><div><div class="section-kicker">04 今日測驗</div><h2 class="h4 section-title mb-1">${stage.quizCount} 題</h2></div><span class="badge badge-soft">通過 ${Math.ceil(stage.quizCount * 0.75)} 題</span></div>
				<p class="small text-body-secondary">題庫只取用 Day 1～${day} 已學內容，練習與測驗不提供振假名。</p>
				<button class="btn ${parts.quiz ? 'btn-success' : 'btn-primary'} w-100 fw-bold" id="open-quiz-button">${parts.quiz ? '✓ 已通過 · 再做一次' : '開始今日測驗'}</button>
			</section>
		</div>
	</div>`;
}
...
export function renderQuiz(day, state) {
	const lesson = lessonForDay(day);
	const stage = stageForDay(day);
	const attempt = state.quizHistory.filter((item) => item.day === day).length + 1;
	const questions = generateQuiz(lesson, stage, attempt);
	const pass = Math.ceil(questions.length * 0.75);
	return {
		questions,
		html: `<div class="row justify-content-center"><div class="col-xl-8 col-lg-9">
			<div class="quiz-topbar mb-4"><button class="lesson-nav-btn" id="back-to-lesson-button"><i class="bi bi-arrow-left"></i></button><div class="flex-grow-1"><div class="section-kicker">DAY ${day} · 第 ${attempt} 次</div><h1 class="h2 fw-bold mb-1">今日測驗</h1><div class="small text-body-secondary">題目不顯示振假名 · 只考 Day 1–${day} 已學內容</div></div><div class="quiz-count"><b>${questions.length}</b><small>題</small></div></div>
			<div class="quiz-rule mb-3"><i class="bi bi-eye-slash"></i><span>送出答案後會顯示中文解析與必要讀音；重新作答會更換部分題目。</span><b>${pass} 題通過</b></div>
			...
			<div class="submit-sticky"><button class="btn btn-primary btn-lg w-100 fw-bold" id="submit-quiz-button">送出 ${questions.length} 題答案</button></div>
		</div></div>`
	};
}
